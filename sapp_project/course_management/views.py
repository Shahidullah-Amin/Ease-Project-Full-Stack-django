...
@login_required
@user_passes_test(student_test, 'student-login')
def student_lessons(request):
    courses = request.user.student.courses.all()

    return render(request , "course_management/student/student_scores.html" , {'courses':courses})
...
def student_login(request):
    ...
    if request.method == 'POST':
        ...
        user_error = "Username or password is incorrect!"

    return render(request, 'course_management/student_login.html', {'user_error': user_error , 'username':username})
...
def student_home(request):
    ...
    for course  in request.user.student.courses.all():
        for room in course.class_room.all():
            rooms.append(room)
    return render(request, 'course_management/student/student_schedule.html',{'lecture_days':DAYS_OF_WEEK_CHOICES , 'rooms':rooms,'lecture_times':LECTURE_START_TIMES})
...
def delete_class(request, id):
    ...
    if ClassRoom.objects.filter(id=id).exists():
        ...
        class_room.delete()
        messages.success(
            request, f'{class_room} has been deleted successfully')
...
def delete_lesson(request, id):
    ...
    if request.META.get('HTTP_REFERER') == request.build_absolute_uri(reverse('lessons')):
        ...
        if Course.objects.filter(id=id).exists():
            ...
            course.delete()
            messages.success(
                request, f"'{course_name}' has been deleted successfully")
    ...
    if Course.objects.filter(id=id).exists():
        ...
        if request.method == 'POST':
            ...
            strcuture_form = CourseStructureForm(
                request.POST, instance=course.course_structure)
            if form.is_valid() and strcuture_form.is_valid():
                form.save()
                strcuture_form.save()
                messages.success(
                    request, f"'{course_name}' edited successfully")
                return redirect(reverse('lessons'))
    ...
    if (Personal.objects.filter(id=int(id)).exists()):
        ...
        if (request.method == 'POST'):
            ...
            if user_form.is_valid() and form.is_valid():
                ...
                teacher.faculty = Faculty.objects.get(
                    pk=int(request.POST.get('faculty')))
                ...
                teacher.save()
                messages.success(
                    request, 'Teacher has been updated successfully')
...
def delete_teacher(request, id):
    ...
    if request.META.get('HTTP_REFERER') == request.build_absolute_uri(reverse('teachers')) or request.META.get('HTTP_REFERER') == request.build_absolute_uri(reverse('delete-confirm', kwargs={'id': id})):
        if User.objects.filter(id=int(id)).exists():
            ...
            user.delete()
            messages.success(
                request, f"'{username}' has been deleted successfully")
            return redirect(reverse('teachers'))
        else:
    ...
    if (request.method == 'POST'):
        ...
        if user_form.is_valid() and password_form.is_valid():
            ...
            if form.is_valid():
                form.save()
                messages.success(
                    request, "Teacher has been added successfully")
                return redirect('teachers')
            else:
                user.delete()
        else:
...
def evaluate_student(request, id=None):
    ...
    if hasattr(request.user, 'personal'):
        ...
        if id:
            ...
            student_ordered_marks = Mark.objects.all().filter(
                course=course).order_by('-marks')
                ...
                for student in students:
                    if student.student_mark.all().filter(course=course).first():
                        ...
                        mark = Mark.objects.create(
                            student=student, course=course, marks=request.POST.get(student.student_no))
                        mark.save()
                    success = True
                if success:
                    messages.success(
                        request, 'Score has been evaluated successfully')
    ...
    if (request.method == 'POST'):
        ...
        if form.is_valid():
            ...
            if course.course_topics.filter(week_no=int(request.POST.get('week_no'))).exists():
                form.add_error('week_no', "Topic with Week No " +
                               request.POST.get('week_no')+" already exists.")
            ...
            topic = CourseTopic(week_no=request.POST.get('week_no'), topic=request.POST.get(
                'topic'), document=request.FILES.get('document'))
            topic.save()
        ...
        if request.method == 'POST':
            if request.POST.get('midterm_exam'):
                ...
                if form.is_valid():
                    form.save()
                    messages.success(
                        request, 'Course has been edited successfully', 'success')
...
def home(request):
    ...
    class_room = ClassRoom.objects.all().filter(
        course__lecturer=request.user.personal)
    ...
    if hasattr(request.user, 'personal'):
        ...
        if selected:
            if selected == 'all':
                ...
                courses = request.user.personal.course_set.filter(
                    semester=selected)
            else:
                courses = request.user.personal.course_set.filter(
                    semester=selected)
        ...
        if selected:
            if selected == 'all':
                ...
                courses = request.user.student.course_set.filter(
                    semester=selected)
            else:
                courses = request.user.student.course_set.filter(
                    semester=selected)
